...
import { TalentService, CandidatDTO, CandidatResponseDTO } from '../../services/talent.service';
  ...
  downloadLettre(lettreId: number, titre: string): void {
    this.talentService.downloadLettre(lettreId).subscribe({
      next: (response) => {
        ...
        // Récupérer le nom du fichier via l'en-tête (Content-Disposition)
        const contentDisposition = response.headers.get('Content-Disposition');
        let fileName = `${titre}_${lettreId}`;
